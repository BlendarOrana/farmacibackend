...
import { X, BadgeCheck, Trash2, Plus, ImagePlus } from "lucide-react";

function AvatarImage({ src, name, size = "w-8 h-8", shape = "rounded-md", fit = "object-contain" }) {
  ...
  if (src && !broken) {
    return (
      <img
        ...
        className={`${size} ${shape} ${fit} bg-white border border-gray-100 p-0.5 shadow-sm`}
      />
    );
  }
  return (
    <div className={`${size} ${shape} bg-gray-900 text-white flex items-center justify-center font-bold text-xs uppercase shadow-md`}>
      {name ? name.charAt(0) : <BadgeCheck size={14} className="opacity-60" />}
    </div>
  );
}
      ...
      <div className="bg-white rounded-[28px] max-w-[420px] w-full flex flex-col shadow-[0_30px_60px_-15px_rgba(0,0,0,0.15)] relative z-10 animate-in zoom-in-95 duration-200 border border-gray-100 overflow-hidden max-h-[85vh]">
        ...
        <div className="px-7 py-5 flex justify-between items-center bg-white border-b border-gray-50 shrink-0 z-20">
          <div className="flex flex-col">
            ...
            <p className="text-[12px] font-bold uppercase tracking-wider text-gray-400 mt-0.5">{brands?.length || 0} marka regjistruara</p>
          </div>
        ...
        <div className="p-5 bg-gray-50/50 shrink-0">
          <form onSubmit={handleCreate} className="flex flex-col gap-3">
            ...
            >
              {saving ? <div className="w-5 h-5 border-2 border-white/50 border-t-white animate-spin rounded-full" /> : <><Plus strokeWidth={2.5} size={18} className="mr-2" /> Shto Brendin</>}
            </button>
          </form>
        </div>
        ...
        <div className="overflow-y-auto flex-1 p-2 custom-minimal-scrollbar bg-white min-h-[180px]">
          {!brands || brands.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 py-10 px-6 text-center">
              <BadgeCheck className="mb-3 opacity-20" size={32} />
              <p className="text-[13px] font-medium leading-relaxed">Nuk keni asnjë brand regjistruar.<br />Fusni një sipër.</p>
            </div>
          ) : (
            <ul className="space-y-1">
              {brands.map((b) => (
                <li key={b.id} className="flex items-center justify-between p-3.5 pl-5 bg-white border border-transparent rounded-[14px] hover:border-gray-100 hover:shadow-[0_2px_10px_-2px_rgba(0,0,0,0.05)] hover:bg-[#fafafa] transition-all group">
                  <div className="flex items-center gap-3">
                    <AvatarImage src={b.image_url} name={b.name} />
                    <span className="text-[15px] font-bold text-gray-800">{b.name}</span>
                  </div>
